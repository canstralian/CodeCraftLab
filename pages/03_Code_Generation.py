...
if not available_models:
    st.warning(
        "No trained models available. Please train a model in the Model Training section."
    )
else:
            ...
            if model_info:
                ...
                with st.expander("Model Details", expanded=False):
                    for key, value in model_info.items():
                        if (
                            key != "id"
                        ):  # Skip ID as it's already shown in the selectbox
                            st.markdown(f"**{key.replace('_', ' ').title()}:** {value}")

        # Generation parameters
        st.markdown("#### Generation Parameters")
        max_length = st.slider(
            "Maximum Length", min_value=50, max_value=500, value=200, step=10
        )
    ...
    with col2:
        ...
        if generate_button and prompt and selected_model:
            with st.spinner("Generating code..."):
                ...
                # Log the generation
                add_log(
                    f"Generated code with model '{selected_model}' (length: {len(generated_code)})"
                )
        ...
        # If there's code history but the generate button wasn't pressed, show the most recent one
            ...
            code_placeholder.code(
                "# Generated code will appear here", language="python"
            )
    ...
    if not st.session_state.code_history:
        ...
        for i, item in enumerate(reversed(st.session_state.code_history)):
            with st.expander(
                f"Generation {len(st.session_state.code_history) - i}: {item['timestamp']}"
            ):
                st.markdown(f"**Model:** {item['model']}")
                st.markdown(
                    f"**Parameters:** Max Length: {item['parameters']['max_length']}, "
